...
class GaussianDiffusionTrainer(nn.Module):
    ...
    def forward(self, x_0):
        ...
        return loss
# 训练其中加入labels
class GaussianDiffusionConditionTrainer(nn.Module):
    def __init__(self, model: nn.Module, beta: Tuple[int, int], T: int):
        super().__init__()
        self.model = model
        self.T = T
        ...
        # calculate and store two coefficient of $q(x_t | x_0)$
        self.register_buffer("sqrt_alphas_bar", torch.sqrt(alpha_t_bar))
        self.register_buffer("sqrt_one_minus_alphas_bar", torch.sqrt(1.0 - alpha_t_bar))
    ...
    def forward(self, x_0, labels):
        ...
        x_t = (extract(self.sqrt_alphas_bar, t, x_0.shape) * x_0 +
               extract(self.sqrt_one_minus_alphas_bar, t, x_0.shape) * epsilon)
...
# 采样中加入labels
class DDPMConditionSampler(nn.Module):
    def __init__(self, model: nn.Module, beta: Tuple[int, int], T: int, w=0.):
        super().__init__()
        self.model = model
        self.T = T
        self.w = w
    ...
    @torch.no_grad()
    def cal_mean_variance(self, x_t, t, labels):
        """
        Calculate the mean and variance for $q(x_{t-1} | x_t, x_0)$
        """
        epsilon_theta = self.model(x_t, t, labels)
        non_epsilon_theta = self.model(x_t, t, torch.zeros_like(labels).to(labels.device))
        epsilon_theta = (1. + self.w) * epsilon_theta - self.w * non_epsilon_theta
    ...
    @torch.no_grad()
    def sample_one_step(self, x_t, time_step: int, labels):
        """
        Calculate $x_{t-1}$ according to $x_t$
        """
        t = torch.full((x_t.shape[0],), time_step, device=x_t.device, dtype=torch.long)
        mean, var = self.cal_mean_variance(x_t, t,labels)
    ...
    @torch.no_grad()
    def forward(self, x_t, labels, only_return_x_0: bool = True, interval: int = 1, **kwargs):
        """
        Parameters:
            x_t: Standard Gaussian noise. A tensor with shape (batch_size, channels, height, width).
            only_return_x_0: Determines whether the image is saved during the sampling process. if True,
        ...
        with tqdm(reversed(range(self.T)), colour="#6565b5", total=self.T) as sampling_steps:
            for time_step in sampling_steps:
                x_t = self.sample_one_step(x_t, time_step, labels)
